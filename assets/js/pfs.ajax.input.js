    ...
    $.extend(ajaxInput.prototype, {
        ...
        watcher: function(id, options) {
            ...
            $("#"+ id).on('change pfs:input-change select2:select krajeeselect2:cleared', function(e) {
                ...
                self.depChange();
                self.$element.find('.temporary').each(function(index, el) {
                    $(el).remove(); 
                });
            });
        ...
        preDepProcess: function(value) {
            var self = this, forceDisabled = false, params = {};
            for(var i = 0; i < self.depLength; i++) {
        ...
        renderListInput: function(type, data) {
            var self = this, $container, $label, $checkbox, formatter,
        ...
        createListElement: function(type, val, text, selected, labelOptions, itemOptions, checked) {
            ...
            $container = $('<div/>', {'class': type +' temporary'});
        ...
        renderText: function(data) {
            var self = this, values, value, sources = [], $valueEl;
            ...
            // create sources
            $.each(data, function(index, val) {
                sources.push({value: index, label: val});
